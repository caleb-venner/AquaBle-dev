...
function renderDeviceCardHeader(
  ...
  return `
    <div class="device-header" style="padding: 16px; border-bottom: 1px solid var(--gray-200);">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
          <button
            ...
            style="padding: 6px 10px; font-size: 16px; background: transparent; border: 1px solid var(--gray-300); border-radius: 6px; cursor: pointer; color: var(--gray-600); transition: all 0.2s;"
            onmouseover="this.style.background='var(--gray-100)'; this.style.borderColor='var(--gray-400)'; this.style.color='var(--gray-900)';"
            onmouseout="this.style.background='transparent'; this.style.borderColor='var(--gray-300)'; this.style.color='var(--gray-600)';"
          >
            ⚙
          </button>
...
function renderDeviceCardFooter(device: CachedStatus & { address: string }): string {
  const connectButtonText = device.connected ? 'Reconnect' : 'Connect';
  const connectButtonClass = device.connected ? 'btn-secondary' : 'btn-primary';

  return `
    <div class="device-footer" style="padding: 16px; border-top: 1px solid var(--gray-200); background: var(--gray-50);">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <button class="btn btn-outline" onclick="window.openDeviceSettings('${device.address}', '${device.device_type}')">
          Settings
        </button>
        <button class="btn ${connectButtonClass}" onclick="window.toggleDeviceConnection('${device.address}')">
          ${connectButtonText}
        </button>
      </div>
    </div>
