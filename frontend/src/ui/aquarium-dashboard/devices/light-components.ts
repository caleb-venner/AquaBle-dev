...
export function renderLightCardStatus(device: DeviceStatus & { address: string }): string {
  ...
  // Fallback: show manual mode status or simple connection state
  return renderLightManualStatus(device, deviceConfig);
}

/**
 * Render light manual mode status or simple connection fallback
 */
function renderLightManualStatus(device: DeviceStatus & { address: string }, deviceConfig: any): string {
  const keyframes = deviceConfig?.last_status?.parsed?.keyframes || [];
  ...
  return `
    <div style="padding: 16px; display: flex; flex-direction: column; gap: 12px;">
      <div style="padding: 12px 16px; background: var(--bg-primary); border-radius: 8px; border-left: 4px solid var(--primary);">
        <div style="font-size: 14px; font-weight: 600; color: var(--text-primary); margin-bottom: 4px;">Light Status</div>
        <div style="font-size: 12px; color: var(--text-secondary); margin-bottom: 8px;">
          Device responds with ${keyframes.length} data point(s)
        </div>
        <div style="font-size: 12px; color: var(--text-primary); font-family: monospace; background: var(--bg-secondary); padding: 8px; border-radius: 4px;">
          ${keyframes.map((kf: any) => `Time: ${kf.hour.toString().padStart(2, '0')}:${kf.minute.toString().padStart(2, '0')} | Level: ${kf.value}%`).join('<br>')}
        </div>
      </div>
    </div>
  `;
