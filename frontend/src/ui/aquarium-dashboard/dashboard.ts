...
export function initializeDashboardHandlers(): void {
  ...
  (window as any).handleRefreshDevice = async (address: string) => {
    ...
    try {
      if (refreshButton) {
        refreshButton.disabled = true;
        refreshButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: block; animation: spin 1s linear infinite;"><path d="M21 12a9 9 0 1 1-6.219-8.56"></path></svg>`;
      }

      const deviceType = deviceStore.getState().devices.get(address)?.status?.device_type || 'light';
      await refreshDeviceStatus(address, deviceType);
  ...
  (window as any).toggleDeviceConnection = async (address: string) => {
    ...
    try {
      ...
      const deviceRecord = zustandState.devices.get(address);
      const isCurrentlyConnected = deviceRecord?.status?.connected;
      const deviceType = deviceRecord?.config?.device_type || 'light';
      ...
      if (isCurrentlyConnected) {
        ...
        const connectedStatus = await connectDevice(address, deviceType);
  ...
  (window as any).saveDeviceCardSettings = async (address: string) => {
    ...
    try {
      ...
      // Reload device configuration to get new name/settings
      const state = deviceStore.getState();
      const isDoser = state.configurations.dosers.has(address);
      await state.actions.refreshDeviceConfig(address, isDoser ? 'doser' : 'light');
