...
export function renderDevTab(): string {
  ...
  // Convert device Map to array, including configuration data for raw payload inspection
    ...
    <div class="device-raw-data-card" style="border: 1px solid var(--gray-200); border-radius: 8px; overflow: hidden;">
      ...
      <div class="device-raw-data-content" id="${deviceId}-content" style="display: none; padding: 16px; background: var(--bg-primary);">
        <div style="margin-bottom: 16px;">
          ...
          <div style="color: var(--gray-500); font-style: italic; padding: 12px; background: var(--gray-50); border-radius: 6px; border: 1px solid var(--gray-200);">Complete device state including raw payloads and parsed configuration data from the backend.</div>
        </div>

        <div style="margin-bottom: 16px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <div style="font-size: 12px; color: var(--gray-500); font-weight: 600;">Raw Status</div>
            <button class="btn btn-outline" style="padding: 2px 8px; font-size: 11px; min-height: 24px;" onclick="navigator.clipboard.writeText(document.getElementById('${deviceId}-json').textContent).then(() => { const b = this; const orig = b.innerText; b.innerText = 'Copied!'; setTimeout(() => b.innerText = orig, 2000); })">Copy</button>
          </div>
          <pre id="${deviceId}-json" style="background: var(--gray-50); padding: 12px; border-radius: 6px; font-size: 12px; overflow-x: auto; margin: 0; border: 1px solid var(--gray-200);">${JSON.stringify(device, null, 2)}</pre>
        </div>
      </div>
    </div>
  `;
