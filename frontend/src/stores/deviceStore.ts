...
interface DeviceStore {
  ...
  actions: {
    ...
    connectToDevice: (address: string, deviceType?: string) => Promise<void>;
...
const storeInitializer: StateCreator<DeviceStore> = (set, get) => ({
  ...
  actions: {
    ...
    initializeStore: async () => {
      ...
      // Stage 3: Auto-connect to all known devices that are currently disconnected
      const devices = get().devices;
      const disconnectedDevices = Array.from(devices.values()).filter(
        device => device.status && !device.status.connected
      );
      
      if (disconnectedDevices.length > 0) {
        console.log(`🔌 Attempting auto-connect for ${disconnectedDevices.length} disconnected devices...`);
        
        // Connect to them all concurrently but don't block the UI load
        disconnectedDevices.forEach(device => {
          const deviceType = device.status.device_type;
          get().actions.connectToDevice(device.address, deviceType)
            .catch(err => console.error(`Auto-connect failed for ${device.address}:`, err));
        });
      }
    },
    ...
    refreshDevice: async (address) => {
      try {
        ...
        // We really should pass device_type, but we don't know it here easily without looking it up
        const device = get().devices.get(address);
        const deviceType = device?.status?.device_type || 'light';
    ...
    connectToDevice: async (address, deviceType = 'light') => {
      try {
        const { postJson } = await import("../api/http");
        await postJson(`api/devices/${encodeURIComponent(address)}/connect?device_type=${encodeURIComponent(deviceType)}`, {});
